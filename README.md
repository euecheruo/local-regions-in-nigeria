...
To ensure consistent data processing and clear visual hierarchy, the following structural rules govern how regions are joined and formatted. This logic handles the "hyphen vs. space" conflict found in native Nigerian place names.

### 1. Structural Separator (The Joiner)

To distinguish between two **Regions** being joined at the same administrative level, an underscore (**_**) is used. This character is chosen because it does not exist in the native naming conventions of any Nigerian language or in official English place names, preventing data "collision."

### 2. Name Integrity (The Rule)

Region names must be preserved exactly as they appear in the source or as intended by their native locale. This includes:
...
* **Compound Region Underscore:** This rule applies strictly to **Regions**. Names representing two or more joined, historically distinct regions on the same administrative level must separate the constituent names with an underscore (_), with no spaces.
...
* *Replacement:* If a special character (like `/` or `&`) is used as a separator between regions, it should be replaced with an underscore (`_`).
...
* **Numerals Only:** A region name cannot consist of only a numerical number.
...
| **LGAs** | **Akoko_Edo** | Joined regions; underscore used as joiner. |
...
| **Wards** | **Ward I_Ward II** | Expanded from "Ward 1 & 2" using underscore. |
